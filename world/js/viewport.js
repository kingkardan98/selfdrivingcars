class Viewport {
    ...
    getMouse(evt, subtractDragOffset = false) {
        let p = new Point((evt.offsetX - this.center.x) * this.zoom - this.offset.x, 
    ...
    #handleMouseWheel(evt) {
        ...
        const step = 0.1;
        this.zoom += dir * step;
        this.zoom = Math.max(1, Math.min(5, this.zoom)); // 1 <= zoom <= 5
    }

    #handleMouseDown(evt) { // 1 = Middle button
        if (evt.button == 1) {
